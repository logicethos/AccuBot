...
public class clsNodeProtoDictionaryShadow : clsProtoDictionaryShadow<clsNode, Proto.API.Node,Proto.API.NodeList>
{
   
    public ProtoPubSub<Proto.API.NodeStatus> NodeStatusSubSub = new ProtoPubSub<NodeStatus>();
    
    public clsNodeProtoDictionaryShadow(): base(Path.Combine(Program.DataPath, "nodes"),
        x=>x.Nodes,
        x=>x.NodeID,
        (x, y) => x.NodeID = y)
    {
